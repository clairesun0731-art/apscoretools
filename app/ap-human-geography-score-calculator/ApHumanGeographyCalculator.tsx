...
type InputField = "mcq" | "frq_1" | "frq_2" | "frq_3";
...
export default function ApHumanGeographyCalculator() {
  const [mcqRaw, setMcqRaw] = useState(30);
  const [frq1, setFrq1] = useState(4);
  const [frq2, setFrq2] = useState(4);
  const [frq3, setFrq3] = useState(4);
  const hasStartedRef = useRef(false);
  const hasInteractedRef = useRef(false);

  const result = useMemo(() => {
    const mcqWeighted = (mcqRaw / 60) * 50;
    const frqRaw = frq1 + frq2 + frq3;
    const frqWeighted = (frqRaw / 21) * 50;
  ...
  function handleInputChange(
    ...
    if (!hasStartedRef.current) {
      ...
      trackEvent("calculator_start", {
        calculator_name: "ap_human_geography_score_calculator",
        page_type: "tool_page",
      });
    }
  ...
  return (
    <section
      aria-label="AP Human Geography score calculator"
    ...
    >
      <div className="chem-calculator-header">
        ...
        <h2>
          <span className="block sm:inline">Enter your AP Human</span>{" "}
          <span className="block sm:inline">Geography section scores</span>
        </h2>

        <p>
          Use your multiple-choice score and three free-response question scores
        ...
        >
          ...
          <div className="chem-input-section">
            ...
            <ScoreField
              ...
              id="aphug-frq-1"
              label="Question 1 - No Stimulus"
              max={7}
              min={0}
              onChange={(value) => handleInputChange("frq_1", value, setFrq1)}
              value={frq1}
            />
            <ScoreField
              helper="Raw score out of 7"
              id="aphug-frq-2"
              label="Question 2 - One Stimulus"
              max={7}
              min={0}
              onChange={(value) => handleInputChange("frq_2", value, setFrq2)}
              value={frq2}
            />
            <ScoreField
              helper="Raw score out of 7"
              id="aphug-frq-3"
              label="Question 3 - Two Stimuli"
              max={7}
              min={0}
              onChange={(value) => handleInputChange("frq_3", value, setFrq3)}
              value={frq3}
            />
          </div>
        </form>
        ...
        >
          ...
          <div className="chem-breakdown-card">
            ...
            <div className="chem-breakdown-row">
              ...
              <strong>{formatWeightedScore(result.mcqWeighted)} / 50</strong>
            </div>
            <div className="chem-breakdown-row">
              <span>FRQ Raw Score</span>
              <strong>{result.frqRaw} / 21</strong>
            </div>
            <div className="chem-breakdown-row">
              <span>FRQ Weighted Score</span>
              <strong>{formatWeightedScore(result.frqWeighted)} / 50</strong>
            </div>
            <div className="chem-breakdown-row">
